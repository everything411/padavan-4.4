...
#define PCIE_KGBL_2		0x04
#define PCIE_DIRECT_MSI_EN	BIT(15)
 ...
 * @has_direct_msi: whether this host supports direct MSI interrupts or not
 ...
 * @lane: lane count
 * @slot: port slot
 * @msi_hwirq_base: MSI hwirq base number for direct msi
 * @msi_parent: MSI parent IRQ domain
...
struct mtk_pcie_port {
	...
	u32 lane;
	u32 slot;
	irq_hw_number_t msi_hwirq_base;
	struct irq_domain *msi_parent;
...
static void mtk_pcie_msi_mask_irq(struct irq_data *data)
{
	struct mtk_pcie_port *port = irq_data_get_irq_chip_data(data);
...
static void mtk_pcie_msi_unmask_irq(struct irq_data *data)
{
	struct mtk_pcie_port *port = irq_data_get_irq_chip_data(data);
...
static void mtk_pcie_msi_irq_ack(struct irq_data *data)
{
	struct mtk_pcie_port *port = irq_data_get_irq_chip_data(data);
	...
	/* send back to sysirq since it only support this interface */
	if (port->msi_parent) {
...
static int mtk_irq_chip_set_affinity(struct irq_data *data,
		const struct cpumask *mask, bool force)
{
	...
	pr_debug("irq [%d] affinity not support\n", data->irq);
	return -EINVAL;
}
...
static struct irq_chip mtk_msi_irq_chip = {
	...
	.irq_ack                = mtk_pcie_msi_irq_ack,
	.irq_set_affinity       = mtk_irq_chip_set_affinity,
	.irq_mask               = mtk_pcie_msi_mask_irq,
	.irq_unmask             = mtk_pcie_msi_unmask_irq,
		...
		msi_alloc_info_t *arg)
{
	...
	if (bit >= MTK_MSI_IRQS_NUM) {
		dev_dbg(port->pcie->dev, "bit [%ld] out of MSI IRQ number range\n", bit);
		return -ENOSPC;
	}
...
static void mtk_pcie_msi_set_desc(msi_alloc_info_t *arg, struct msi_desc *desc)
{
	...
	arg->desc = desc;
	hwirq = &port->msi_hwirq_base;

	/* set args by device node style, which will be passed to sysirq */
	fwspec->param_count = 3;
	fwspec->param[1] = *hwirq;
	fwspec->param[2] = IRQ_TYPE_LEVEL_LOW;

	(*hwirq)++;
}

static void mtk_pcie_msi_finish(msi_alloc_info_t *arg, int retval)
{
	struct pci_dev *dev = msi_desc_to_pci_dev(arg->desc);
	struct mtk_pcie_port *port = mtk_pcie_find_port(dev->bus, dev->devfn);
	...
	/*
	 * sysirq will not help to set irq type at runtime,
	 * we need to trigger it after irq mapping have been created.
	 */
	if (port->msi_parent) {
		for_each_msi_entry(desc, arg->desc->dev)
			irq_set_irq_type(desc->irq, IRQ_TYPE_LEVEL_LOW);
	}
}
		...
		struct msi_domain_info *info, unsigned int virq)
{
	...
	if (!test_bit(d->hwirq, port->msi_irq_in_use)) {
		dev_dbg(port->pcie->dev, "trying to free unused MSI#%lu\n", d->hwirq);
	} else {
...
static struct msi_domain_ops mtk_msi_domain_ops = {
	...
	.msi_finish = mtk_pcie_msi_finish,
...
static int mtk_pcie_allocate_msi_domains(struct mtk_pcie_port *port)
{
	...
	if (port->pcie->soc->has_direct_msi) {
		irq_node = of_irq_find_parent(dev->of_node);
		if (!irq_node) {
			dev_dbg(dev, "irq parent node not found\n");
			goto legacy_msi;
		}

		port->msi_parent = irq_find_host(irq_node);
		if (!port->msi_parent) {
			dev_dbg(dev, "pcie interrupt-parent domain not found\n");
			goto legacy_msi;
		}

		msi_vbase = platform_get_irq(pdev, 0);
		data = irq_get_irq_data(msi_vbase);
		if (!data)
			goto legacy_msi;

		port->msi_hwirq_base = irqd_to_hwirq(data);
		irq_dispose_mapping(msi_vbase);

		goto direct_msi;
	}

legacy_msi:
	port->msi_parent = NULL;
...
direct_msi:
	...
	port->msi_domain = pci_msi_create_irq_domain(fwnode, info,
						     port->msi_parent);
	if (!port->msi_domain) {
		dev_dbg(dev, "failed to create MSI domain\n");
		return -ENOMEM;
	}
...
static void mtk_pcie_enable_msi(struct mtk_pcie_port *port)
{
	...
	if (port->msi_parent) {
		val = readl(port->base + PCIE_KGBL_2);
		writel(PCIE_DIRECT_MSI_EN | val, port->base + PCIE_KGBL_2);
	}
}
...
static irqreturn_t mtk_pcie_intr_handler(int irq, void *data)
{
	...
	if (IS_ENABLED(CONFIG_PCI_MSI) && !port->msi_parent) {
...
static const struct of_device_id mtk_pcie_ids[] = {
	...
	{ .compatible = "mediatek,leopard-pcie", .data = &mtk_pcie_soc_mt7629 },
	{},
};
