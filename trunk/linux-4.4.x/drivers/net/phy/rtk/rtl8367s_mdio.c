...
struct rtl_sw_data {
	void (*init_gsw)(void);
	int (*vlan_config)(int want_at_p0);
	int (*swconfig_init)(void (*reset_func)(void));
};
...
extern int rtl8363nb_swconfig_init( void (*reset_func)(void) );
#endif
...
static int rtl8363nb_vlan_config(int want_at_p0)
{
	rtk_vlan_cfg_t vlan1, vlan2;
...
static void init_rtl8367s(void)
{
	rtl8367s_hw_init();
	set_rtl8367s_sgmii();
	set_rtl8367s_rgmii();
...
static void set_rtl8363nb_rgmii(void)
{
	rtk_port_mac_ability_t mac_cfg;
	rtk_mode_ext_t mode;
...
static void init_rtl8363nb(void)
{
	rtl8367s_hw_init();
	set_rtl8363nb_rgmii();
}

struct rtl_sw_data rtl8367s_data = {
	.init_gsw = init_rtl8367s,
	.vlan_config = rtl8367s_vlan_config,
	.swconfig_init = rtl8367s_swconfig_init,
};

struct rtl_sw_data rtl8363nb_data = {
	.init_gsw = init_rtl8363nb,
	.vlan_config = rtl8363nb_vlan_config,
	.swconfig_init = rtl8363nb_swconfig_init,
};

// bleow are platform driver
static const struct of_device_id rtk_gsw_match[] = {
	{ .compatible = "mediatek,rtk-gsw", .data = &rtl8367s_data },
	{ .compatible = "mediatek,rtl8363nb-gsw", .data = &rtl8363nb_data },
	{},
};
...
static int rtk_gsw_probe(struct platform_device *pdev)
{
	...
	match = of_match_device(rtk_gsw_match, &pdev->dev);
	soc = (struct rtl_sw_data *)match->data;
	soc->init_gsw();
...
#ifdef CONFIG_SWCONFIG
		soc->swconfig_init(soc->init_gsw);
#else
		soc->vlan_config(0);
#endif
	}
