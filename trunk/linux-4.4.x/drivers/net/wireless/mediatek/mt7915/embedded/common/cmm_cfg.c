...
IOCTL_ATE_ERROR:
		...
		/* handle for SIOCGIWRATEQ */
	{
		...
		if (WMODE_CAP_AX(wdev->PhyMode)) {
			HePhyMode = pAd->ApCfg.MBSSID[pObj->ioctl_if].wdev.HEPhyMode;
			Antenna = wlan_config_get_he_tx_nss(wdev);
			BW =  HePhyMode.field.BW;
...
VOID StatMt7915RxRateToString(RTMP_ADAPTER *pAd, CHAR *Output, UINT32 RawData)
{
	...
	if (phy_mode > MODE_HE_MU) {
		sprintf(Output + strlen(Output), "Undefined for %d\n", phy_mode);
		return;
	}
	...
	if (phy_mode == MODE_CCK) {
		...
		if (((phy_mode == MODE_VHT) && (rate > 9)) || ((phy_mode >= MODE_HE) && (rate > 11)))
			sprintf(Output + strlen(Output), "Incorrect NSS%d_MCS%d, ", ((nsts + 1) / (stbc + 1)), rate);
		else
			sprintf(Output + strlen(Output), "NSS%d_MCS%d, ", ((nsts + 1) / (stbc + 1)), rate);
	}

	sprintf(Output + strlen(Output), "%s, ", bwMode[bandwidth]);

	if (phy_mode == MODE_HTMIX || phy_mode == MODE_HTGREENFIELD || phy_mode == MODE_VHT)
		sprintf(Output + strlen(Output), "%cGI, ", (guard_interval & 0x1) ? 'S' : 'L');
	if (phy_mode >= MODE_HE)
		sprintf(Output + strlen(Output), "%s GI, ", HeGi[guard_interval]);

	sprintf(Output + strlen(Output), "%s", phyMode[phy_mode]);

	if (phy_mode >= MODE_HTMIX) {
		sprintf(Output + strlen(Output), "%s", (stbc) ? ", STBC, " : ", ");
		sprintf(Output + strlen(Output), "%s", FecCoding[coding]);
	}
		...
		if ((pSCSCtrl->OneSecTxByteCount[u1BandIdx]) * 9 <  pSCSCtrl->OneSecRxByteCount[u1BandIdx]) {
			...
			for (j = 1; VALID_UCAST_ENTRY_WCID(pAd, j); j++) {
				...
				if ((IS_ENTRY_CLIENT(pEntry) && pEntry->Sst == SST_ASSOC) || IS_ENTRY_WDS(pEntry) || IS_ENTRY_APCLI(pEntry)) {
					/* Active STA - Tput > 2Mbps */
					if (pEntry->OneSecTxBytes + pEntry->OneSecRxBytes > 250000) {
						pSCSCtrl->ActiveSTAIdx[u1BandIdx] = j;
						ActiveSTA++;
					}

					if ((ActiveSTA == 1) && (pSCSCtrl->ActiveSTAIdx[u1BandIdx] == j)) {
					...
					if (ActiveSTA != 1) {
						...
						("Band%u:  Wcid %u, ActiveSTA %u, eTput %u\n",
							u1BandIdx,
							j,
							ActiveSTA,
							eTput
...
INT SendApccci2fwMsg(RTMP_ADAPTER *pAd, struct _MT_WIFI_COEX_APCCCI2FW *apccci2fw_msg)
{
	...
	MTWF_LOG(DBG_CAT_COEX, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("%s:\n", __func__));
...
#endif
	MTWF_LOG(DBG_CAT_COEX, DBG_SUBCAT_ALL, DBG_LVL_OFF,
		("%s: apccci2fw_msg->len=%d\n", __func__, apccci2fw_msg->len));
	call_wifi_md_coex_notifier(FW_DRIVER_APCCCI, (void *)apccci2fw_msg);
...
INT SetTxPowerBoostCtrl(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	...
	return TxPwrUpCtrl(pAd, ucBandIdx, u1PwrUpCat, cPwrUpValue);
}
	...
	if (arg) {
		...
		case MCLI_FORCE_RPS_CFG:
			if (band_idx < DBDC_BAND_NUM) {
				...
				sprintf(pAd->mcli_ctl[band_idx].force_proc_rps[ETH0_RPS_FILE], "%d", op);
				sprintf(pAd->mcli_ctl[band_idx].force_proc_rps[BAND0_RPS_FILE], "%d", op2);
				sprintf(pAd->mcli_ctl[band_idx].force_proc_rps[BAND1_RPS_FILE], "%d", op3);
				sprintf(pAd->mcli_ctl[band_idx].force_proc_rps[TXFREE_IRQ_FILE], "%d", op4);
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR,
					("%s: proc_rps [%s %s %s %s]\n", __func__,
